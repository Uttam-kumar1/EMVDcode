...
def load_cvrd_data(shift, noisy_level, scene_ind, frame_ind, xx, yy):
	...
	# CRVD_dataset/indoor_raw_gt/indoor_raw_gt_scene{}/scene{}/ISO{}/frame{}_clean_and_slightly_denoised.tiff
...
def load_eval_data(noisy_level, scene_ind):
	...
	in_data = torch.from_numpy(input_batch.copy()).permute(0, 3, 1, 2)  # 1 * (4*25) * 128 * 128
	gt_raw_data = torch.from_numpy(gt_raw_batch.copy()).permute(0, 3, 1, 2)  # 1 * (4*25) * 128 * 128
